			...
			xml2js.parseString(req.data,{mergeAttrs: true, explicitArray: false, ignoreXmlns: true, ignoreAttrs: true},function(err,json){
				if (err) {
					...
					try {
						...
						try {
							//GEOFF commented out the following
							//options.outputs = service.implementation[actionName].call(service,inputs) || {};

							//GEOFF added the following
							service.implementation[actionName].call(service,inputs, function( inError, inResult ) {
								options.outputs = inResult;
...
var handlePostEvent = function(req,rsp,peer){
	...
	if (split.length==2) {
		...
		if (service) {
//GEOFF
			xml2js.parseString(req.data,{strict: false, mergeAttrs: true, explicitArray: false, ignoreXmlns: true},function(err,json){
				if (json) {
					var data = {};
